eif_delta_s <- function(y, a, g, e, pi, pistar, gamma0, mu0, Q0, gamma1, mu1, Q1, t0) {
  ...
  part_j1_mu <- map(1:tt, ~eif_delta_s_partj_mu(., y, a, gamma1, mu1, pi, pistar, Q1)) %>%
    bind_cols
  part_j0_mu <- map(1:tt, ~eif_delta_s_partj_mu(., y, a, gamma0, mu0, 1-pi, 1-pistar, Q0)) %>%
    bind_cols
...
eif_delta_s_partj_mu <- function(j, y, a, gamma, mu, pi, pistar, Q) {
  ...
  apq_gp_j <- a[,j]*Q[,j]*pistar[,j]/gamma[,j]/pi[,j]

  out <- ayp_gp_jm1*apq_gp_j*(y[,j] - mu[,j])
...
eif_delta_s_partj_q <- function(j, y, a, gamma, mu, pi, pistar, Q) {
  ...
  if (j == 1) {
    ayp_gp_jm1 <- 1
  } else ayp_gp_jm1 <- apply(aypistar_gammapi[,1:(j-1),drop=FALSE], 1, prod)

  out <- ayp_gp_jm1*pistar[,j]*(mu[,j+1]*Q[,j+1] - Q[,j])
  if (any(is.na(out))) browser()
  out
}
