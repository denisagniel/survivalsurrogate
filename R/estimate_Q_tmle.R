estimate_Qj_tmle <- function(data, folds, id, x, g, a_jm1, a_j, y_jm1, y_j, sbar_jm1, mu_jp1, Q_jp1, gammabar_j, e, gval, j, lrnr, epsilon = 1e-7) {
  at_risk_data <- data
  if (!is.null(a_jm1)) {
  ...
  if (any(is.na(at_risk_data$muQ))) browser()
  Q_j <- estimate_cont(at_risk_data, folds, id, c(x, sbar_jm1), 'muQ', 'include_in_training', lrnr, Q_nm)
  ...
  tmle_fm <- glue::glue('muQ ~ -1 + offset(qlogis({Q_nm}))')
...
estimate_Q_tmle <- function(data, folds, id, x, g, all_a, all_y, all_s, all_mu, all_gamma, e, gval, lrnr, slim = FALSE) {
  tt <- length(all_y)
  tt_s <- length(all_s)
  ...
  for (t in (tt-1):0) {
    if (t == 0) {
      Q_dat <- estimate_Qj_tmle(Q_dat, folds, id, x, g, NULL, NULL, NULL, NULL, NULL, all_mu[1], paste0('Q', gval, '_1'), NULL, e, gval, t, lrnr)
    } else if (t == 1) {
      Q_dat <- estimate_Qj_tmle(Q_dat, folds, id, x, g, NULL, all_a[1], NULL, all_y[1], NULL, all_mu[t+1], paste0('Q', gval, '_', t+1), NULL, e, gval, t, lrnr)
    } else {
