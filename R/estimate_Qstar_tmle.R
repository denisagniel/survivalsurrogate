estimate_Qstar_yj_tmle <- function(data, folds, id, x, g, a_jm1, a_j, y_jm1, y_j, sbar_jm1, Q_jp1, gammabar_j, pistar_jm1, pi_jm1, e, gval, j, lrnr, epsilon = 1e-7) {
  at_risk_data <- data
  if (!is.null(a_jm1)) {
  ...
  updated_data <- mutate(rowwise(updated_data), wt_j = ifelse(gval == 1, !!sym(g)/!!sym(e)*!!sym(a_j)/prod(c_across(gammabar_j))*prod(c_across(pistar_jm1))/prod(c_across(pi_jm1)), (1-!!sym(g))/(1-!!sym(e))*!!sym(a_j)/prod(c_across(gammabar_j))*prod(c_across(pistar_jm1))/prod(c_across(pi_jm1))),
...
estimate_Qstar_sj_tmle <- function(data, folds, id, x, g, a_jm1, a_j, y_jm1, y_j, sbar_jm1, Q_jp1, gammabar_j, pistar_jm1, pi_jm2, e, gval, j, lrnr, epsilon = 1e-7) {
  at_risk_data <- data
  if (!is.null(a_jm1)) {
    at_risk_data <- filter(at_risk_data, !!sym(a_jm1) == 1)
  }
  ...
  if (!is.null(a_j)) {
    ...
    at_risk_data <- mutate(at_risk_data, include_in_training = 1,
                           a_j = 1)
    a_j <- 'a_j'
  }
  ...
  Q_j <- estimate_cont(at_risk_data, folds, id, c(x, sbar_jm1), 'Q_s', 'include_in_training', lrnr, Q_nm)
  updated_data <- inner_join(at_risk_data, Q_j, by = id)

  updated_data <- mutate(rowwise(updated_data), wt_j = ifelse(gval == 1, 1/!!sym(e)*!!sym(a_j)/prod(c_across(gammabar_j))*prod(c_across(pistar_jm1))/prod(c_across(pi_jm2)), 1/(1-!!sym(e))*!!sym(a_j)/prod(c_across(gammabar_j))*prod(c_across(pistar_jm1))/prod(c_across(pi_jm2))),
                         !!Q_nm := pmax(pmin(!!sym(Q_nm), 1-epsilon), epsilon)) ### this is a kluge
  updated_data <- ungroup(updated_data)
...
estimate_Qstarj_tmle <- function(data, folds, id, x, g, a_jm1, a_j, y_jm1, y_j, sbar_jm1, Q_jp1, gammabar_j, e, pistar_jm1, pi_jm1, pi_jm2, gval, j, lrnr, epsilon = 1e-7) {
  data_j <- estimate_Qstar_sj_tmle(data = data,
                                   ...
                                   lrnr = lrnr,
                                   epsilon = epsilon)
  out_j <- estimate_Qstar_yj_tmle(data = data,
                                  folds = folds,
                                  id = id,
                                  x = x,
                                  g = g,
...
estimate_Qstar_tmle <- function(data, folds, id, x, g, all_a, all_y, all_s, all_gamma, pistar, pi, e, gval, lrnr, slim = FALSE) {
  tt <- length(all_y)
  tt_s <- length(all_s)
  ...
  for (t in tt:1) {
    if (t == 0) {
      Q_dat <- estimate_Qstarj_tmle(Q_dat, folds, id, x, g, NULL, NULL, NULL, NULL, NULL, paste0('Q', gval, '_1'), NULL, NULL, NULL, NULL, e, gval, t, lrnr)
    } else if (t == 1) {
      Q_dat <- estimate_Qstarj_tmle(Q_dat, folds, id, x, g, NULL, all_a[1], NULL, all_y[1], NULL, paste0('Q', gval, '_', t+1), NULL, NULL, NULL, NULL, e, gval, t, lrnr)
    } else if (t == 2) {
