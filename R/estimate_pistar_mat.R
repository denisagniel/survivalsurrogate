...
estimate_pistar_mat <- function(data, folds, id, x, g, all_a, all_y, all_s, lrnr, t0 = length(all_s), slim = FALSE) {
  ...
  pistar_js <- map(1:t0, function(t) {
    if (t == 1) {
      estimate_pistar_j(data, folds, x, g, all_a[t],  all_y[t], NULL, t+1, lrnr)
    } else {
      estimate_pistar_j(data, folds, x, g, all_a[t],  all_y[t], all_s[1:t-1], t+1, lrnr)
    }
  ...
  for (j in 1:t0) {
    out_pistar <- left_join(out_pistar, pistar_js_slim[[j]])
  }
  if (tt > t0+1) {
    for (ttt in (t0+2):tt) {
  ...
  if (slim) {
    ...
    return(left_join(data, out_pistar))
  }

}

estimate_pistar_j <- function(data, folds, x, g, a_j, y_j, sbar_j, j, lrnr) {
  at_risk_data <- data
  if (!is.null(a_j)) {
  ...
  estimate_binary(at_risk_data, folds, c(x, sbar_j), g, 'include_in_training', lrnr, paste0('pistar_', j))
}
